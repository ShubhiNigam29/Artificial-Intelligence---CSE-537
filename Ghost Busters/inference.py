...
#Authors: RishabhKhot_SBUID_112682983_ShubhiNigam_SBUID_112672816
...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            allPossible = util.Counter()
            """
            Getting all the positions of the pacman if there is no noisy distance
            """
            allPossible[self.getJailPosition()] = 1.0
        else:
            """
            For each legal position get the distance of the location and the pacman position so that the next move can be made
            """
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        uniformInitialized = []
        """
        If there are number of particles greater than 0 then check with the legalPosition
        Add the position to the uniform search and reduce the num of Particles
        """
        while n > 0:
            if n > len(legalPosition):
                uniformInitialized += legalPosition
                n -= len(legalPosition)
            else:
                """
                Add the legal poitions to the uniform search and make n to 0
                """
                uniformInitialized += legalPosition[0:n-1]
                n = 0
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        """
        Getting permutations of all the legal positions and then shuffling the permutation so that a random position is selected
        """
        permutations = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
        random.shuffle(permutations)
        permutationLength = len(permutations)
        number = self.numParticles
        self.particles = []
        """
        Traverse the permuation till the number becomes greater the permuation length
        """
        while number >= permutationLength:
            self.particles += permutations
            number -= permutationLength
        self.particles += permutations[0: number - 1]
    ...
    def observeState(self, gameState):
        ...
        for index in range(self.numGhosts):
            """
            Check the indices of the jail and append if the index doesn't have anything
            """
        ...
        """
        For each particle traverse throught the indices of ghosts and then checking the number if ghosts in that index
        based on the above information update the index and get the true distance of the pacman from the ghosts
        """
        for particle in self.particles:
            ...
            for index in range(self.numGhosts):
                if index not in jailIndices:
                    ...
                    ghostPosition = particle[index]
                    trueDistance = util.manhattanDistance(ghostPosition, pacmanPosition)
                    """
                    Multiplying the current probability with the true distance of the model
                    """
                    currentProbability *= model[trueDistance]
            counter[particle] += currentProbability
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                """
                 A list of ghost positions now loop through and update each entry in newParticle
                """
                newPosition = getPositionDistributionForGhost(setGhostPositions(gameState, oldGPositions), i, self.ghostAgents[i])
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        """
        A belief network defines a factorization of the joint probability distribution, 
        where the conditional probabilities form factors that are multiplied together.
        """
        beliefDist = util.Counter()
        for particle in self.particles:
            """
            Here is the belief distribution for each particle is incremented by 1
            """
            beliefDist[particle] += 1.0
        """
        The Role of Normalization in the Belief Propagation Algorithm.
        The belief propagation algorithm, which is an exact procedure to compute these marginals when the underlying graph 
        is a tree, has gained its popularity as an efficient way to approximate them in the more general case.
        """
